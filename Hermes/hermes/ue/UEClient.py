...
class UEClient:
    ...
    def sendMessage(self, msgs, applyTemplates=False, suppressBodyPrint = False, templates=None):
        url = self.ueurl
        result = []
    ...
    def sendFromFile(self, msgfile, applyTemplates=True, suppressBodyPrint = False, templates=None):
        print(f"UE ({self.instance})","sendFromFile", msgfile)
        with open(msgfile) as json_file:
            msg = json.load(json_file)
        return self.sendMessage(msg, applyTemplates=applyTemplates, suppressBodyPrint=suppressBodyPrint, templates=templates)
    ...
    def checkConnection(self):
        ...
        (sc, result) = self.sendFromFile(os.path.join(self.internalMessageRoot,"checkConnectivity.json"),applyTemplates=False, suppressBodyPrint = True)
        ...
        (sc, result) = self.sendFromFile(os.path.join(self.internalMessageRoot, "checkWorld.json"), applyTemplates=True, suppressBodyPrint = True)
